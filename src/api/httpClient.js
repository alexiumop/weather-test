...
const httpClient = {
    ...
    async getActualWeatherByCityName(cityName) {
        const URL_ACTUAL_WEATHER = `https://api.openweathermap.org/data/2.5/weather?q=${cityName}&appid=${API_KEY}&units=metric`;
        const ACTUAL_WEATHER =  await this.axiosInstance(URL_ACTUAL_WEATHER);
        return ACTUAL_WEATHER;
    },
    async getForecastWeather(cityName) {
        const URL_FORECAST_WEATHER = `https://api.openweathermap.org/data/2.5/forecast?q=${cityName}&appid=${API_KEY}&units=metric`;
